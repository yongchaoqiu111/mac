...
stdin, stdout, stderr = ssh.exec_command("mysql -uroot -p'WalletBackup2026!' wallet_backup -e \"SELECT id, game, TIME(match_time) as time, status FROM virtual_matches WHERE DATE(match_time) = CURDATE() ORDER BY match_time LIMIT 10;\"")
print(stdout.read().decode('utf-8'))
